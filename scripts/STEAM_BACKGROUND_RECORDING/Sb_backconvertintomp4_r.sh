...
# Interactive prompts for delay and bitrate
interactive_prompts() {
    echo ""
    echo "=== Interactive Mode ==="
    echo "Timeline file specified: $TIMELINE_FILE"

    # Get delay input
    while true; do
        read -p "Enter audio delay in milliseconds (e.g., -300, 0, 500): " DELAY_MS

        if [[ -z "$DELAY_MS" ]]; then
            echo "Delay cannot be empty. Using default: 0"
            DELAY_MS="0"
            break
        elif [[ "$DELAY_MS" =~ ^-?[0-9]+$ ]]; then
            break
        else
            echo "Invalid input. Please enter a number (e.g., -300, 0, 500)"
        fi
    done

    # Get bitrate input
    while true; do
        read -p "Enter video bitrate (e.g., 3M, 2000k, 5M): " BITRATE

        if [[ -z "$BITRATE" ]]; then
            echo "Bitrate cannot be empty. Using default: copy (original bitrate)"
            BITRATE="0"
            break
        elif [[ "$BITRATE" =~ ^[0-9]+[kM]?$ ]] || [[ "$BITRATE" == "0" ]]; then
            break
        else
            echo "Invalid input. Please enter a valid bitrate (e.g., 3M, 2000k)"
        fi
    done
...
# If timeline file is specified as second argument, enable interactive mode
if [ "$INTERACTIVE_MODE" -eq 1 ] && [ -n "$TIMELINE_FILE" ] && [ -f "$TIMELINE_FILE" ]; then
    interactive_prompts
fi

# Use defaults if not set interactively
if [ -z "$DELAY_MS" ]; then
    DELAY_MS="0"
fi
...
# Function to build and run ffmpeg command
run_ffmpeg_conversion() {
    local cmd_args=("$@")
    echo "Running: ffmpeg ${cmd_args[@]}"
    ffmpeg "${cmd_args[@]}"
    return $?
}
...
if [ "$DELAY_MS" -lt 0 ]; then
    ...
    # Add chapters metadata if available (comes before inputs)
    if [ -n "$CHAPTERS_META_FILE" ] && [ -f "$CHAPTERS_META_FILE" ]; then
        FFMPEG_CMD+=(-i "$CHAPTERS_META_FILE")
        CHAPTERS_INDEX=0
        # Video input will be at index 1, audio at index 2
    else
        CHAPTERS_INDEX=-1
        # Video input will be at index 0, audio at index 1
    fi

    # Add inputs: delayed video first, then original audio
    # Video input (index 1 or 0 depending on chapters)
    FFMPEG_CMD+=(-itsoffset "$DELAY_SEC" -i "$MPD_FILE")
    # Audio input (index 2 or 1 depending on chapters)
    FFMPEG_CMD+=(-i "$MPD_FILE")

    # Determine indices based on whether chapters are present
    ...
    run_ffmpeg_conversion "${FFMPEG_CMD[@]}"
    FFMPEG_EXIT=$?
