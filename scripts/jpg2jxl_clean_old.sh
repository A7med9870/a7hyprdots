...
if [ $# -eq 0 ]; then
    ...
    echo "Finds duplicate JPEG/PNG files that have JXL equivalents and removes the originals after confirmation"
    exit 1
fi
...
while IFS= read -r jxl_file; do
    ...
    for ext in png PNG; do  # FIXED: Changed 'webp' to 'png'
