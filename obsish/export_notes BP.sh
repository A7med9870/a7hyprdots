...
find "$NOTES_DIR" -type f -name "20[0-9][0-9]-[0-1][0-9]-[0-3][0-9].md" | while read -r file; do
    ...
    # Convert Markdown to HTML using Pandoc and prepend the timestamp
    pandoc --standalone --css="$DARK_CSS" "$file" -o "${file%.md}.html"
    sed -i "1i $timestamp" "${file%.md}.html"

    # Convert HTML to PNG using wkhtmltopdf
    wkhtmltopdf --enable-local-file-access "${file%.md}.html" "$output_png"
