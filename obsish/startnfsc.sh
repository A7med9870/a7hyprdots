...
# Try to find Proton automatically if the default path doesn't work
if [[ -f "/usr/share/steam/compatibilitytools.d/proton-ge-custom/proton" ]]; then
    PROTON_PATH="/usr/share/steam/compatibilitytools.d/proton-ge-custom/proton"
elif [[ -f "$HOME/.steam/root/compatibilitytools.d/proton-ge-custom/proton" ]]; then
    PROTON_PATH="$HOME/.steam/root/compatibilitytools.d/proton-ge-custom/proton"
elif [[ -f "$HOME/.local/share/Steam/compatibilitytools.d/proton-ge-custom/proton" ]]; then
    PROTON_PATH="$HOME/.local/share/Steam/compatibilitytools.d/proton-ge-custom/proton"
else
    echo "ERROR: Proton not found! Please install Proton-GE or adjust the PROTON_PATH variable."
    exit 1
fi
...
echo "Prefix directory: $PFX_DIR"

# Run the game
cd "$GAME_DIR"
echo "Launching $GAME_EXE with Proton from $GAME_DIR..."
